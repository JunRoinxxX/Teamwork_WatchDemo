...
void SystemInit(void) {
    ...
    RCC->PLLCFGR = 0x00000000;//0x24003010;
...
//    RCC->PLLCFGR |= (uint32_t)(12 << 22); //select HSE as PLL source clock   HSECLK is 16MHz

//    RCC->PLLCFGR |= (uint32_t)(6 << 0); //select PLLM==16
//    RCC->PLLCFGR |= (uint32_t)(100 << 6); //select PLLN==192   VCOCLK= HSECLK*(PLLN/PLLM)=16*(192/16)=192

//    RCC->PLLCFGR |= (uint32_t)(4 << 24); //set PLLQ==3   PLLVOUTCLK = VCOCLK/PLLQ= 192/3 = 64MHz
...
//    RCC->CFGR |= RCC_CFGR_PPRE1_DIV2;//(uint32_t)(4 << 10); //APB1 presc is 2 APB1CLK=32MHz
//    RCC->CFGR |= RCC_CFGR_PPRE2_DIV1;//(uint32_t)(0 << 13); //APB2 presc is 2  APB2CLK=32MHz
...
void SystemCoreClockUpdate(void) {
    uint32_t tmp = 0, pllvco = 0, pllp = 2, pllsource = 0, pllm = 6;
